...
import miniProject.server.exception.JwtTokenMalformedException;
...
public class JwtUtil {
    ...
    public UserVo getUserDataFromToken(final String token){

        try{
            Claims body = Jwts.parser().setSigningKey(jwtSecret).parseClaimsJws(token).getBody();
            UserVo userVo = new UserVo();
            userVo.setUsername(body.getSubject());
            userVo.setUserRole(body.get("role").toString());
